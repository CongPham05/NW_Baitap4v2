...
const GroupTable: React.FC<GroupTableProps> = ({ colCurren, dataList, columnStates }) => {
    ...
    if (colCurren === "status") {
        ...
        return (
            <div className='overflow-x-hidden dark:bg-slate-800 bg-[#f6f8fa]  flex flex-col pb-40 gap-3 '>
                {newColumns.map((column) => (
                    <div key={column.id} className='dark:border-slate-600 bg-white border-t'>
    ...
    if (colCurren === "inProgress") {
        const newColumns = prioritys.map(priority => {
    ...
    if (colCurren === "size") {
        const newColumns = sizes.map(size => {
